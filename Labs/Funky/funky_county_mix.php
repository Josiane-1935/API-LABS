...
function update_array(&$value, $key)
{
  ...
  //fix the two counties with '-':Mandera and Nyamira
...
function get_url_of_county(string $county_name)
{
  $nice_name = preg_replace('/[^A-Za-z]/','',$county_name);
  $county_web = "http://".$nice_name.'.go.ke';
  return $county_web;
}
function get_data_as_table($county_array)
{
  echo '<table border=1>';
...
function getPartyDistribution($county_result)
{
  return array_count_values(array_column($county_result,4));
}

$county_result = splitter($county_data);
get_data_as_table($county_result);
print_r(getPartyDistribution($county_result));
